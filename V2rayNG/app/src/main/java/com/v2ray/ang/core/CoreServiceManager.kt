...
object CoreServiceManager {
    ...
    private fun launchCore(service: Service, vpnInterface: ParcelFileDescriptor?, isReload: Boolean = false) {
        ...
        // The daemon owns session time; UI recreation must not reset the connection timer.
        if (!isReload) {
            MmkvManager.encodeSettings(AppConfig.CACHE_CONNECTION_STARTED_AT, android.os.SystemClock.elapsedRealtime())
        }
